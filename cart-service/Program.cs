...
using QuickBite.Cart.Middlewares;
using QuickBite.Cart.Repository;
...
Log.Information("Starting QuickBite Cart Service...");
...
try
{
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        options.SwaggerDoc("v1", new OpenApiInfo
        {
            ...
            Version     = "v1",
            Description = "Cart management — add/remove items, promo codes, single-restaurant enforcement, price snapshots.",
    ...
    if (app.Environment.IsDevelopment())
    {
        ...
        app.UseSwaggerUI(c =>
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "QuickBite Cart Service v1");
    ...
    Log.Information("QuickBite Cart Service started successfully on {Environment}", app.Environment.EnvironmentName);
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "QuickBite Cart Service failed to start");
}
finally
{
    Log.CloseAndFlush();
